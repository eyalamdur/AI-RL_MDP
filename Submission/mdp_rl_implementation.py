...
def state_appears_before(state: tuple, episode_steps: dict, last_idx: int):
    for i in range(last_idx):
        s, reward, action, actual_action = episode_steps[i]
        if s == state:
            return True
    return False

'''not used:'''
def avgV(results: list):
    if len(results) == 0:
        return 0
    else:
        return results[0]
...
def mc_algorithm2(
    ...
    # ====== YOUR CODE: ======
    Returns = {(r,c): [] for r in range(num_rows) for c in range(num_cols)} # every state has a list of results
    V = np.zeros((num_rows, num_cols)) # initialize Utility array

    # Generate an episode following the requested policy
    for episode_index, episode_gen in enumerate(sim.replay(num_episodes=num_episodes)):
        episode_steps = {} # dictionary maps step_index to step values
        # fill dictionary
        for step_index, step in enumerate(episode_gen):
            #print(f"@@@@    episode {episode_index}   @@@@@")
            episode_steps[step_index] = step

        G = 0
        # Loop for each step of episode
        for t in reversed(range(len(episode_steps))):
            state, reward, action, actual_action = episode_steps[t]
            #print(f"Step {t}: state={state}, reward={reward}, action={action}, actual_action={actual_action}")
            if reward is None:  # encountered a "WALL"
                print("found wall")
                V[state[ROW], state[COL]] = None
                continue

            G = gamma * G + reward
            # If state t doesn't appear earlier in the dictionary
            if not state_appears_before(state, episode_steps, t):
                Returns[state[ROW],state[COL]].append(G)
                V[state[ROW],state[COL]] = average(Returns[state[ROW],state[COL]])
...
def first_visit(state: tuple, episode_steps: dict, index: int):
    for i in range(index):
        s, _, _, _ = episode_steps[i]
        if s == state:
            return False
    return True